...
export function fontMetrics(font: FontBundle, pixel_size: number, more_line_gap = 0.0) {
	...
	const line_height = Math.round(cap_scale * (font.ascent + font.descent + font.line_gap) + more_line_gap)
...
export function charRect(pos: number[], font: FontBundle, font_metrics: FontMetrics, font_char: FontChar, kern = 0.0) {
	...
	const left = pos[0] + font.aspect * scale * (font_char.bearing_x! + kern - font.ix)
	const right = left + font.aspect * scale * (g[2] - g[0])
	const p = [left, top, right, bottom]

	// Advancing pen position
	const new_pos_x = pos[0] + font.aspect * scale * (font_char.advance_x! + kern)
