...
export function createProgram(gl: WebGL2RenderingContext, vertex: string, fragment: string, attribs: Attrib[]) {
	...
	function make_unf_set(name: string, location: WebGLUniformLocation, type: number) {
		if (type == gl.FLOAT) {
	...
	function make_unf_setv(name: string, location: WebGLUniformLocation, type: number) {
		if (type == gl.FLOAT) {
			return function (value: number[]) {
...
export function initAttribs(gl: WebGL2RenderingContext, attribs: Attrib[], offset = 0) {
	let stride = 0
 ...
 */
export async function loadFont(gl: WebGL2RenderingContext, name: string, path = '/fonts'): Promise<Font> {
	const fontTexture = loadTexture(gl, `${path}/${name}.png`, gl.LUMINANCE, false)
	const fontBundle = await fetch(`${path}/${name}.json`).then((res) => res.json())
