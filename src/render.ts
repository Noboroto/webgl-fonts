...
export function createRenderer(gl: WebGL2RenderingContext) {
	...
	function layout({ font, font_size, text }: Layout) {
		...
		const fmetrics = fontMetrics(font.fontBundle, font_size_scaled, font_size_scaled * 0.2)

		// Laying out the text
		str_res = writeString(text, font.fontBundle, fmetrics, [0, 0], vertex_array)
	...
	function render({
		...
		if (prev_resolution[0] != canvas_width || prev_resolution[1] != canvas_height) {
			do_update = true
		...
		const screen_mat = new Float32Array([ws, 0, 0, 0, hs, 0, dx * ws, dy * hs, 1])
